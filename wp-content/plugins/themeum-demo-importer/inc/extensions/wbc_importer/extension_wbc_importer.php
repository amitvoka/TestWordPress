...
if ( !class_exists( 'ReduxFramework_extension_wbc_importer' ) ) {
    ...
    class ReduxFramework_extension_wbc_importer {
                ...
                foreach ( $imports as $import ) {
                    ...
                    if ( $import['type'] == "d" && !empty( $import['name'] ) ) {
                        ...
                        foreach ( $import['files'] as $file ) {
                            ...
                            case 'theme_data.json':
                                $this->wbc_import_files['wbc-import-'.$x]['theme_data'] = $file['name'];
                                break;
        ...
        function add_importer_section() {
            ...
            $wbc_importer_label = trim( esc_html( apply_filters( 'wbc_importer_label', __( 'Travelkit Demo Importer', 'framework' ) ) ) );

            $wbc_importer_label = ( !empty( $wbc_importer_label ) ) ? $wbc_importer_label : __( 'Travelkit Demo Importer', 'framework' );

            $this->parent->sections[] = array(
                'id'     => 'wbc_importer_section',
                'title'  => $wbc_importer_label,
                'desc'   => '<p class="description">'. apply_filters( 'wbc_importer_description', esc_html__( 'Select Your desire demo content. Make sure install one demo content.', 'framework' ) ).'</p>',
